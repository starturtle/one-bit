...
int run_as_script(const one_bit::ArgumentParser& in_params)
{
  ...
  {
    logging::LogStream::instance() << logging::Level::ERR << "Usage: one-bit [-gui=\"true|false\"] [-infile=<infile>] [-outfile=<outfile>] [-rows=<rows> -cols=<cols>]" << logging::Level::OFF;
    logging::LogStream::instance() << logging::Level::ERR << "  Note: infile and outfile are only optional if gui is true!" << logging::Level::OFF;
  ...
  if (pixelator.empty())
  {
    logging::LogStream::instance() << logging::Level::ERR << "Input file [" << in_params.get_input_file() << "] is empty!" << logging::Level::OFF;
    return errors::WRONG_INPUT_FILE;
  }
  
  logging::LogStream::instance() << logging::Level::NOTE << pixelator.imageStats() << logging::Level::OFF;

  // convert to binary image
  if (!pixelator.prepare())
  {
    logging::LogStream::instance() << logging::Level::ERR << "Failed to prepare!" << logging::Level::OFF;
  ...
  {
    logging::LogStream::instance() << logging::Level::ERR << "Failed to pixelate!" << logging::Level::OFF;
